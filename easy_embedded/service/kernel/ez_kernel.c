...
{
    EZTRACE("ezKernel_Initialization()");
    ...
    EZINFO("Kernel service is initialized OK");
}
    ...
    uint32_t task_data_size)
{
    ...
    EZTRACE("ezKernel_AddTask()");

    if (kernel.is_busy == false && function != NULL && task != NULL)
    {
        kernel.is_busy = true;
        EZDEBUG("kernel is not busy, start adding task...");
        ...
        if (status == ezFAIL)
        {
            EZDEBUG("Add task fail");
            ezKernel_ResetKernelTask(task);
        }
        else
        {
            EZDEBUG("Add task success");
#if(DEBUG_LVL == LVL_TRACE)
    ...
    if(IS_LIST_EMPTY(&kernel.executed_tasks) == false)
    {
        ...
        do
        {
            ...
            if (task->exec_cnt_down <= 0)
            {
                ...
                {
#if(DEBUG_LVL == LVL_TRACE)
                    EZTRACE("executing...time stamp = %d", kernel.current_tick);
                    ezKernel_PrintTaskInfo(task);
#endif /* DEBUG_LVL == LVL_TRACE */
...
static void ezKernel_ResetKernelTask(struct ezKernelTask *task)
{
    EZTRACE("ezKernel_ResetKernelTask()");
...
void ezKernel_PrintTaskInfo(struct ezKernelTask *task)
{
    ...
    {
        EZTRACE("------------------------------------");
        EZTRACE("delay = %d millis", task->delay_millis);
        EZTRACE("count down = %d millis", task->exec_cnt_down);
        EZTRACE("data =@0x%p", task->task_data);
        EZTRACE("data size = %d", task->task_data_size);
        EZTRACE("function = @0x%p", task->function);
        EZTRACE("------------------------------------\n\n");
    }
}
#endif /* DEBUG_LVL == LVL_TRACE */
